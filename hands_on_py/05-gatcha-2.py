...
def failures_before_first_success(outcomes):
    """
    outcomes: list of 0/1
    returns:
      failures_before_success: banyak gagal sebelum sukses pertama
      success_found: 1 jika ada sukses, 0 jika tidak ada sukses
    """
...
def simulate_gatcha(balance: int, bet_amount: int, p_win: float, rng: np.random.Generator):
    ...
    outcomes = (rng.random(n_tries) < p_win).astype(int).tolist()  # 1=win, 0=lose
    n_wins = int(sum(outcomes))
    revenue = n_wins * 1000
    new_balance = balance - cost + revenue

    fails_before_success, success_found = failures_before_first_success(outcomes)
...
def add_history_record(result: dict, balance_before_round: int):
    ...
    st.session_state.history.append({
        ...
        "fails_before_success": result["fails_before_success"],
        "success_found": result["success_found"],
    })
...
def plot_failures_before_success_timeseries(df_round: pd.DataFrame):
    fig, ax = plt.subplots(figsize=(10, 4))

    if df_round.empty:
        ax.set_title("Time Series Jumlah Gagal Sebelum Satu Sukses")
        ax.set_xlabel("Round ke-")
        ax.set_ylabel("Jumlah gagal")
        ax.grid(True, alpha=0.3)
        return fig

    x = df_round["round"].values
    y = df_round["fails_before_success"].values
...
def plot_failures_before_success_histogram(df_round: pd.DataFrame):
    fig, ax = plt.subplots(figsize=(8, 4))

    if df_round.empty:
        ax.set_title("Histogram Gagal Sebelum Satu Sukses")
        ax.set_xlabel("Jumlah gagal")
        ax.set_ylabel("Frekuensi")
        ax.grid(True, alpha=0.3)
        return fig

    vals = df_round["fails_before_success"].dropna().astype(int).values
    if len(vals) == 0:
        ax.set_title("Histogram Gagal Sebelum Satu Sukses")
        ax.set_xlabel("Jumlah gagal")
        ax.set_ylabel("Frekuensi")
    ...
    if st.button("Try", use_container_width=True):
        ...
        if result is None:
            ...
            st.success(
                ...
                f"net {result['net']:+} | gagal sebelum sukses pertama {result['fails_before_success']}"
            )
...
with g1:
    st.pyplot(plot_failures_before_success_timeseries(df_round))

with g2:
    st.pyplot(plot_failures_before_success_histogram(df_round))
...
if df_round.empty:
    ...
    show_df_round = df_round[[
        "round", "n_tries", "wins", "losses", "fails_before_success",
        "success_found", "cost", "revenue", "net", "balance"
    ]].copy()
    show_df_round["success_found"] = show_df_round["success_found"].map({1: "Ada", 0: "Tidak"})
    show_df_round.columns = [
        "Round", "Jumlah Coba", "Menang", "Kalah", "Gagal Sebelum Sukses Pertama",
        "Sukses Ditemukan?", "Biaya", "Pemasukan", "Net", "Saldo Akhir"
    ]
